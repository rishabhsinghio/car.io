...
@export var handle_sensitivity : float = 40.0   # Increased max sensitivity
@export var handle_return_speed : float = 5.0
...
func _gui_input(event: InputEvent) -> void:
	if event is InputEventScreenDrag:
		...
		var local_current_usable: Vector2
		if local_current.distance_to(event.position) < 100:
			local_current_usable = local_current * 10
		else:
			local_current_usable = local_current

		var local_last = last_drag_position - pivot_offset
		var local_last_usable: Vector2

		if local_last.distance_to(event.position) < 100:
			local_last_usable = local_last * 10
		else:
			local_last_usable = local_last

		var angle_diff = local_current_usable.angle_to(local_last_usable) * 2

		# sensitivity increased, dividing less makes stronger steering
		rotation_degrees = lerp(
			rotation_degrees,
			rotation_degrees - rad_to_deg(angle_diff) * handle_sensitivity / 8.0,
			.07
		)
...
# call this to play horn, only if horn_enabled is true.
func play_horn() -> void:
	if horn_player:
		horn_player.play(.2)
		

# convert the handle's rotation to the output rotation.
